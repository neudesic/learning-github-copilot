...
- Navigate to the `sample-code\SimpleFullStack\DotnetCoreApi\copilot-sample.Api\Services\CategoryService.cs` folder in Visual Studio 2022.
...
- In the `CategoryService.cs` file, add a comment inside the `AddCategoryAsync` method:
  ...
  Press Enter and wait a few seconds to see the Copilot suggestions. Review the suggestions and apply them.
